...
| 5 | Bulk image regeneration / re-watermarking w BO może odpalić `actionWatermark` wielokrotnie dla tego samego `(id_product, id_image)` w obrębie jednego requestu. W normalnym flow upload-pojedynczego-obrazu PS fires hook RAZ per uploaded image (po watermarkingu wszystkich sizes wewnętrznie), więc spam nie pojawia się w happy path — ale bulk-regen go produkuje. | Hook-level dedup per `(id_product, id_image)` w obrębie request lifecycle (spec §"Duplicate hook fires for the same image are deduplicated"). Pierwszy invocation per key wykonuje upstream call; kolejne są no-op. Non-cover obrazy NIE są pomijane — dedup działa tylko na *tym samym* obrazie, nie cross-image. |
...
| 7 | Pierwszy realny smoke test wywołań przeciw `https://qamera.ai/api/v1/plugin` może odsłonić bugi Fazy 1 (auth, retry, idempotency) nieujawnione w mocku | Operator-driven smoke jest **wymagany** przed merge (`tasks.md §11`). Credentials z untracked local source (po remediation leaka — patrz §11 blocker note); NIE z tracked `CLAUDE.md`. Mock'owane testy nie wystarczą. |
