...
When the `actionWatermark` PrestaShop hook fires with parameters `id_image` and `id_product`, and `Configuration::get('QAMERAAI_AUTO_REGISTER_PRODUCTS')` evaluates truthy, and the bookkeeping row for that `(id_product, id_shop)` has `status='pending'` or `status='error'`, the module SHALL register the product upstream by uploading the primary image via the Qamera AI Plugin API. The flow is: (1) resolve the bookkeeping row, (2) resolve the primary image for the product, (3) request a presigned upload URL via `QameraApiClient::requestUpload()`, (4) PUT the image bytes to that URL, (5) call `QameraApiClient::registerImage` with `product_ref` from the row, `source_url` from the presigned response, and `product_metadata` built from the row's snapshot columns, (6) persist the upstream response to the bookkeeping row. When the toggle evaluates falsy, the hook SHALL be a no-op. When the bookkeeping row already has `status='registered'` and `qamera_product_id IS NOT NULL`, the call SHALL still register the new image but SHALL OMIT `product_metadata` from the request (the product is already known upstream).
...
- **WHEN** `actionWatermark` fires with `id_product=42, id_image=99` and the upstream `POST /plugin/images` returns 201 with `{product_id: 'abc-uuid', image_id: 'img-uuid', ...}`
- **THEN** the request to `/plugin/images` carries `product_ref='ps:1:42'`, `source_url=<presigned PUT target>`, and `product_metadata={display_name:'Widget', sku:'WDG-001', description:'hello'}`
...
- **THEN** the request to `/plugin/images` carries `product_ref='ps:1:42'` and `source_url=...` but does NOT include `product_metadata`
...
- **THEN** only one upstream `POST /assets/upload`, one PUT, and one `POST /plugin/images` are issued
