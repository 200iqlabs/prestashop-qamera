...
1. **Install path**: `Installer::createSchema()` gains a third `CREATE TABLE IF NOT EXISTS` statement inline next to the existing `qamera_product_link` and `qamera_packshot_link` creates (no separate `createWebhookDeliveryTable()` helper — extracting one would add a layer without behaviour change). Idempotent like the others.
...
- **Q1: Should the front controller class live in `controllers/front/webhook.php` (PS convention) or `src/Controller/Front/`?** Initial reading is that PS's legacy dispatcher only finds front controllers in the legacy folder. To be confirmed during implementation by reading PS 8 / 9 source — if a Symfony-registered front controller is supported, prefer `src/Controller/Front/` for PSR-4 consistency. Either way, `WebhookRequestHandler` in `src/Webhook/` is where real logic lives.
  - **Resolved (Phase 4.1 implementation):** Legacy `controllers/front/webhook.php` with class `QameraaiWebhookModuleFrontController extends ModuleFrontController`. The Symfony route loader in `config/routes.yml` is admin-only here; adding a storefront Symfony route would require expanding the loader scope, which is out of scope for a security-sensitive change. All real logic lives in `src/Webhook/WebhookRequestHandler` (framework-free, fully unit-tested).
