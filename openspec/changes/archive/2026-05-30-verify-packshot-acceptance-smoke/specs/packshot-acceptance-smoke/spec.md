<!-- Verification-only capability. No plugin code is added; these requirements
     assert that the already-shipped `packshot-acceptance` plugin behavior is
     proven end-to-end against the live backend with the gate ON. Each scenario
     is an operator smoke step (credit-spending; live container only). -->
