...
Fix: when the bookkeeping row is already `status='registered'` with a non-empty `qamera_asset_id` **and the primary image is unchanged** (same resolved `id_image` as last sync), the service SHALL skip the upload+register entirely and leave `qamera_asset_id` intact. A new upload happens only on first registration or when the resolved primary `id_image` changes.

Change-detection signal for v1 = the resolved primary `id_image`. Rejected (follow-up): hashing the bytes to detect in-place image replacement under the same `id_image` — real but rare for a re-save; documented as out of scope.
