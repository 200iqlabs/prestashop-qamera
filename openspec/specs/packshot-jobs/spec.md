# packshot-jobs Specification

## Purpose

Maintains the local `ps_qamera_packshot_job` bookkeeping table that mirrors the lifecycle of every job submitted to the Qamera AI API from the BO UI. Provides the submitter pipeline (client-side `packshot_external_ref` generation, upstream submission, chunked bulk handling, post-2xx persistence), the cost calculator that pre-flights credit usage from cached `/pricing`, and the typed repository the BO controllers and webhook handlers read/write through. This capability is the persistence + business-logic layer behind the BO Products grid and Jobs history surfaces — UI rendering belongs to `qamera-bo-ui`, upstream HTTP belongs to `qamera-api-client`, webhook-driven state transitions belong to `webhook-handler`.
## Requirements
