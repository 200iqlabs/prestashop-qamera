...
When the dispatcher routes a `job.completed` delivery, the JobCompletedHandler SHALL parse the payload's `external_ref` into `(shopId, productId, imageId)`, then verify-and-heartbeat `ps_qamera_product_link` for `(id_shop=shopId, id_product=productId)` by bumping `last_synced_at` and `updated_at` to `NOW()`. The heartbeat MUST NOT modify the Phase-3-owned columns `status`, `qamera_product_id`, or `last_error_message`. If no `ps_qamera_product_link` row matches, the handler SHALL log a `warning` and return without writing a packshot row. Otherwise the handler SHALL UPSERT a row in `ps_qamera_packshot_link` keyed on `qamera_packshot_id` from the payload with: `status='ready'`, `qamera_job_id=<payload.job_id>` (NULL if omitted), `id_shop=shopId`, `id_product=productId`, `qamera_packshot_ref=<deterministic ref derived from shopId, productId, qamera_packshot_id>`, `last_synced_at=NOW()`, `updated_at=NOW()`, `last_error_message=NULL`, `created_at=NOW()` on insert.
